...
export default function EditListingPage() {
  ...
  const handleScrape = async () => {
    ...
    setScraping(true);
    try {
      const res = await fetch("/api/scrape", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: scrapeUrl.trim() }) });
      const data = await res.json();
      if (res.ok) {
        setForm((prev) => ({
          ...prev,
          title: data.title || prev.title, price: data.price ? String(data.price) : prev.price,
          mainImage: data.mainImage || prev.mainImage, sellerName: data.sellerName || prev.sellerName,
          description: data.description || prev.description, sourceUrl: data.sourceUrl || scrapeUrl.trim(),
        }));
      } else { alert(data.error || "Failed to load data"); }
